...
namespace TrainingAndDietApp.Infrastructure.Repositories
{
    ...
    public class GymRepository : IGymRepository
    {
        ...
        =>  await _context.Gyms.Where(g=>g.IsAccepted == true).Include(g => g.Address).ToListAsync(cancellationToken: cancellationToken);
        ...
        public async Task<List<Gym>> GetAllGymsAdminAsync(bool status, CancellationToken cancellationToken)
        => await _context.Gyms.Where(g=>g.IsAccepted == status).Include(g => g.Address).ToListAsync(cancellationToken: cancellationToken);
        ...
        public async Task<List<Gym>> GetMentorActiveGymsAsync(int idUser, CancellationToken cancellationToken)
        {
            return await _context.Trainer_Gyms
                .Where(tg => tg.IdTrainer == idUser && tg.Gym.IsAccepted == true)
