    ...
    public class UserServiceDeprecated : IUserServiceDeprecated
    {
        ...
        public async Task<List<TrainingPlanNameDto>> GetTrainerTrainingPlans(int id_trainer)
        {
            ...
            if (trainingPlans.Count == 0)
                throw new NotFoundException("There are no trainingPlans with given trainer_id");
        ...
        public async Task<PageResult<MentorDto>> GetMentors(string roleName, UserQuery query)
        {
            ...
            if (!string.IsNullOrEmpty(query.GymCityPhrase))
            {
                baseQuery = baseQuery.Where(u =>
                    u.TrainerGyms.Any(g => g.Gym.Address.City.ToLower().Contains(query.GymCityPhrase.ToLower())));
            }

            if (!string.IsNullOrEmpty(query.GymNamePhrase))
            {
                baseQuery = baseQuery.Where(u =>
                    u.TrainerGyms.Any(g => g.Gym.Name.ToLower().Contains(query.GymNamePhrase.ToLower())));
            }
        ...
        public async Task<MentorWithOpinionDto> GetMentorWithOpinionsById(string roleName, int id)
        {
            ...
            var users = await _context.Users
                .Where(u => u.IdUser == id && (u.Role.Name == roleName || "Dietician-Trainer" == u.Role.Name)).Select(
                    trainer =>
                        new MentorWithOpinionDto
                        {
                            ...
                            TotalRate = trainer.MentorOpinions.Any() == true
                                ? trainer.MentorOpinions.Average(o => o.Rate)
                                : 0m,
        ...
        public async Task<bool> UserIsDietician(int dieticianId)
        {
